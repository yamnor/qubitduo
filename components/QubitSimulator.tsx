    ...
    className={`w-20 h-20 rounded-full flex items-center justify-center text-2xl font-bold cursor-pointer
                ${
                  state === "0"
                    ...
                    : state === "+"
                    ? "bg-gradient-superposition text-white"
                    : state === "-"
                    ? "bg-gradient-superposition text-white"
...
const QubitSimulator = () => {
  ...
  const [timeData, setTimeData] = useState<
    Array<{ alice: number; bob: number; entangled: number }>
  >([]);
  ...
  const toggleQubit = (
    qubit: string,
    setQubit: React.Dispatch<React.SetStateAction<string>>
  ) => {
  ...
  const histogramData = Object.entries(stateDistribution).map(
    ([state, count]) => ({
      state,
      count,
    })
    ...
    <div className="w-full flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="w-full max-w-7xl relative">
        ...
        <div className="flex flex-row flex-wrap justify-center items-center space-x-4 mb-2">
          <div className="flex flex-col items-center">
            ...
            <QuantumBit
              state={alice}
              onClick={() => toggleQubit(alice, setAlice)}
              name="Alice"
              isNoisy={isNoisy}
            />
            <QuantumGate
              name="H"
              onClick={() => applyHadamardGate(alice, setAlice)}
              color="bg-green-500"
            />
            <button
          ...
          <div className="flex flex-col items-center">
            ...
            <QuantumBit
              state={bob}
              onClick={() => toggleQubit(bob, setBob)}
              name="Bob"
              isNoisy={isNoisy}
            />
            <QuantumGate
              name="H"
              onClick={() => applyHadamardGate(bob, setBob)}
              color="bg-green-500"
            />
            <button
              className="px-4 py-2 rounded bg-purple-500 hover:bg-purple-600 text-white font-bold mt-2"
        ...
        <div className="flex flex-col md:flex-row w-full md:space-x-4">
          <div className="hidden md:flex md:w-1/2 mb-4 flex-col items-center">
          ...
          <div className="hidden md:flex md:w-1/2 mb-4 flex-col items-center">
            <h3 className="text-xl text-gray-500 mb-2 text-center">
              Histogram ({measurementCount} times)
            </h3>
            <div className="w-full h-80">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <SwipeableViews enableMouseEvents className="md:hidden">
      ...
      {isHelpModalOpen && (
        <Modal onClose={() => setIsHelpModalOpen(false)}>
          ...
          <p className="mb-4">
            This is a quantum entanglement simulator. You can manipulate the
            states of two qubits (Alice and Bob) and apply quantum gates to
            observe their behavior.
          </p>
          <h3 className="text-lg font-semibold mb-2">Usage:</h3>
          <ul className="list-disc list-outside mb-4 mx-4">
            <li>
              Click on the qubits (Alice or Bob) to toggle their states between
              0 and 1.
            </li>
            <li>
              Use the H button to apply a Hadamard gate and create a
              superposition state.
            </li>
            <li>Use the CNOT button to entangle the qubits.</li>
            <li>
              Adjust the noise level using the slider to simulate environmental
              noise.
            </li>
          </ul>
          ...
          <ul className="list-disc list-outside mb-4 mx-4">
            <li>
              GitHub:{" "}
              <a
                href="https://github.com/yamnor/qubitduo"
                target="_blank"
          ...
          <p className="text-sm text-gray-500">
            Developed by: Nori Yamamoto (
            <a
              href="https://x.com/yamnor"
              target="_blank"
              className="text-blue-500 hover:text-blue-700"
            >
              @yamnor
